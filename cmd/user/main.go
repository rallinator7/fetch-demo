...
import (
	...
	"github.com/rallinator7/fetch-demo/internal/user"
...
func ServeHTTP(logs Logger) (func() error, *http.Server) {
	...
	payerStream := environment.GetVariableFatal("USER_STREAM", logs)
	...
	publisher := user.NewPublisher(&pub, payerStream)

	control := user.NewController(&repo, &publisher, logs)
	handler := user.NewHandler(&control)
